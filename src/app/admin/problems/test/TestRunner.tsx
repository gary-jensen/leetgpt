...
import { testAllProblems, TestProblemResult } from "@/lib/actions/adminTestActions";
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-muted-foreground mt-1">
						Validate that all problems&apos; passingCode passes all test cases
					</p>
				</div>
					...
					<Table>
						...
						<TableBody>
							{results.map((problemResult) => {
								...
								const totalFailedCases = problemResult.languages.reduce(
									(sum, l) => sum + l.failedTestCasesCount,
									0
								);
											...
											<div className="flex flex-col gap-1">
												{problemResult.languages.map((lang) => (
													<Badge
														key={lang.language}
														variant={
															lang.passed ? "default" : "destructive"
														}
													>
														{lang.language}
													</Badge>
										...
										<TableCell>
											{totalFailedCases > 0 && (
												<Button
													variant="outline"
													size="sm"
													onClick={() => copyFailedTestCases(problemResult)}
												>
													Copy Failed Cases
												</Button>
											)}
