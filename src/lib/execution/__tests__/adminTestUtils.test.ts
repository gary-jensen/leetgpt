import { validatePassingCode, validateAllPassingCodes } from "../adminTestUtils";
...
describe("Admin Test Utils", () => {
	...
	it("should validate passingCode for Valid Palindrome problem", async () => {
		const palindromeProblem = algoProblems.find(p => p.id === "valid-palindrome");
	...
	it("should validate all problems' passingCode", async () => {
		...
		expect(result.validProblems).toEqual(expect.arrayContaining(expectedIds));
	});
});
